      ...
      <div style={{ padding: 16, flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: '#7dd3c7', margin: 0 }}>{hazard.location.address}</p>
          </div>
          <SeverityBadge value={hazard.severity} />
        </div>
        <p style={{ fontSize: 12, color: '#7dd3c7', margin: '8px 0' }}>{hazard.distance.toFixed(1)} km away • {timeAgo(hazard.timestamp)}</p>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
          <span style={{ fontSize: 12, color: '#7dd3c7' }}>{hazard.reporter}</span>
          <button type="button" onClick={() => onAction?.(hazard.id)} style={{ background: '#00c9a7', color: '#021c1a', border: 'none', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
