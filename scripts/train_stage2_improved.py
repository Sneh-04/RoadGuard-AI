#!/usr/bin/env python3
"""
STAGE 2: HAZARD SUBTYPE CLASSIFICATION (Speedbreaker vs Pothole)
Research-grade training script with reproducibility, class balancing, and logging.

Modified: Feb 28, 2026
Improvements:
- Deterministic random seeding
- Class weight balancing (critical for severe imbalance)
...
# augmentation utilities for training-time transformations
...
LOGS_DIR = os.path.join(BASE_DIR, "logs", "stage2")
...
# ====== FILTER HAZARDS ONLY (Classes 1, 2) ======
print("Filtering hazard-only samples (classes 1=Speedbreaker, 2=Pothole)...")
train_mask = np.isin(y_train, [1, 2])
val_mask = np.isin(y_val, [1, 2])
test_mask = np.isin(y_test, [1, 2])

X_train_hazard = X_train[train_mask]
y_train_hazard = y_train[train_mask]
X_val_hazard = X_val[val_mask]
y_val_hazard = y_val[val_mask]
X_test_hazard = X_test[test_mask]
y_test_hazard = y_test[test_mask]

print(f"Filtered X_train shape: {X_train_hazard.shape}")
print(f"Filtered y_train shape: {y_train_hazard.shape}")
print(f"Filtered X_val shape: {X_val_hazard.shape}")
print(f"Filtered y_val shape: {y_val_hazard.shape}")
print(f"Filtered X_test shape: {X_test_hazard.shape}")
print(f"Filtered y_test shape: {y_test_hazard.shape}")
print()

# ====== REMAP LABELS (1->0, 2->1) ======
print("Remapping labels (1=Speedbreaker->0, 2=Pothole->1)...")
y_train_bin = (y_train_hazard == 2).astype(np.int32)
y_val_bin = (y_val_hazard == 2).astype(np.int32)
y_test_bin = (y_test_hazard == 2).astype(np.int32)

train_dist = np.bincount(y_train_bin)
val_dist = np.bincount(y_val_bin)
test_dist = np.bincount(y_test_bin)

print(f"Train distribution: Class 0 (Speedbreaker): {train_dist[0]}, Class 1 (Pothole): {train_dist[1]}")
print(f"Val distribution:   Class 0 (Speedbreaker): {val_dist[0]}, Class 1 (Pothole): {val_dist[1]}")
print(f"Test distribution:  Class 0 (Speedbreaker): {test_dist[0]}, Class 1 (Pothole): {test_dist[1]}")

if len(train_dist) == 2:
    imbalance_ratio = train_dist[0] / train_dist[1]
    print(f"Class imbalance ratio (Speedbreaker:Pothole): {imbalance_ratio:.2f}:1")
print()

# ====== COMPUTE CLASS WEIGHTS ======
print("Computing class weights (CRITICAL for severe imbalance)...")
...
print(f"  Speedbreaker weight: {class_weight_dict[0]:.4f}")
print(f"  Pothole weight: {class_weight_dict[1]:.4f}")
print()

# ====== BUILD MODEL ======
print("Building Stage-2 CNN model...")
model = Sequential([
...
callbacks = [
    ...
    ModelCheckpoint(
        filepath=os.path.join(MODEL_DIR, "stage2_subtype_v2_checkpoint.keras"),
        monitor="val_loss",
        save_best_only=True,
        verbose=1
    ),
...
history = model.fit(
    augment_batch(X_train_hazard), y_train_bin,
    validation_data=(X_val_hazard, y_val_bin),
    epochs=50,
    batch_size=32,
...
# ====== SAVE FINAL MODEL ======
final_model_path = os.path.join(MODEL_DIR, "stage2_subtype_v2.keras")
...
test_loss, test_acc = model.evaluate(X_test_hazard, y_test_bin, verbose=0)
print(f"Test Loss: {test_loss:.4f}, Test Accuracy: {test_acc:.4f}")
print()

# ====== COMPUTE METRICS ======
print("Computing classification metrics...")
y_pred_proba = model.predict(X_test_hazard, verbose=0)
...
print(classification_report(y_test_bin, y_pred, target_names=["Speedbreaker", "Pothole"]))
print()
...
cm_path = os.path.join(RESULTS_DIR, "stage2_confusion_matrix.png")
...
history_path = os.path.join(RESULTS_DIR, "stage2_history.json")
...
metrics = {
    ...
    "class_distribution": {
        "train": {"class_0_speedbreaker": int(train_dist[0]), "class_1_pothole": int(train_dist[1])},
        "val": {"class_0_speedbreaker": int(val_dist[0]), "class_1_pothole": int(val_dist[1])},
        "test": {"class_0_speedbreaker": int(test_dist[0]), "class_1_pothole": int(test_dist[1])}
    },
    "class_weights": class_weight_dict,
    "imbalance_ratio": float(imbalance_ratio) if len(train_dist) == 2 else None,
    "hyperparameters": {
        "epochs": 50,
        "batch_size": 32,
        "optimizer": "adam",
...
metrics_path = os.path.join(RESULTS_DIR, "stage2_metrics.json")
...
axes[0].set_title('Stage-2: Training and Validation Loss')
axes[0].legend()
...
axes[1].set_title('Stage-2: Training and Validation Accuracy')
axes[1].legend()
axes[1].grid(True, alpha=0.3)

curves_path = os.path.join(RESULTS_DIR, "stage2_training_curves.png")
...
fold_num = 1
for train_idx, val_idx in skf.split(X_train_hazard, y_train_bin):
    print(f"\nFold {fold_num}/5")
    
    X_fold_train, X_fold_val = X_train_hazard[train_idx], X_train_hazard[val_idx]
    y_fold_train, y_fold_val = y_train_bin[train_idx], y_train_bin[val_idx]

    # augment training fold
    X_fold_train_aug = augment_batch(X_fold_train)
    ...
    fold_model.compile(optimizer=Adam(learning_rate=1e-3), loss="binary_crossentropy", metrics=["accuracy"])
    
    fold_model.fit(
        X_fold_train_aug, y_fold_train,
...
cv_path = os.path.join(RESULTS_DIR, "stage2_cv_results.json")
